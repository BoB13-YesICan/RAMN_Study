...
void get_attack_variables() {
    // Reset variables to default values && set Field Descriptor to blocking mode
    attack_code = 0;
    canid = 0;
    time_diff = 0;
    interface[0] = '\0';
    ...
    while (1) {
        ...
        scanf("%d", &attack_code);
        if (attack_code >= 1 && attack_code <= 10) break;
    ...
    printf("Selected attack_code=%d, canid=0x%03X, time_diff=%d ms\n", attack_code, canid, time_diff);
}
